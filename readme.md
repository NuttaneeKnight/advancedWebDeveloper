...
__Sample Code for 3D animation button effect__
...
___Rotations___ Using CSS to rotate things
...
.container {
display: flex;
} ___Hence this will become a FLEX CONTAINER___ see the definition below.
...
* flex-direction: column; > making the content into a column.
  ___When it's a column the cross axis moves horizontally with the flex items. While the main axist move vertically___
...
___This won't changed___ Because it has a flex wrap that made more sence for it to wrap inside the flex container
...
___When you set the "flex-wrap" inside the flex container then it will be wrapped inside the size that you set and the container won't force the size (conflicts)___
...
* Defines how space is distributed between items in flex container
  ___Along the cross axis___ cross axis = top to bottom
